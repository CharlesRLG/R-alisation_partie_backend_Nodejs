...
async function startMongo(opts = { args: null }) {
  ...
  const appDir = process.env.MONGO_DIR || path$4.resolve(os$1.tmpdir(), mongoFormula.name);
  const dataDir = path$4.resolve(appDir, "data", instanceName, mongoFormula.name);
  const logsDir = path$4.resolve(appDir, "logs", instanceName, mongoFormula.name);
  const logFile = path$4.resolve(logsDir, "logs.txt");
  const sourceDir = path$4.resolve(appDir, "source", mongoFormula.name, mongoFormula.version, platform.name);
  const sourceFileName = `${mongoFormula.name}-${mongoFormula.version}-${platform.name}` + path$4.extname(platform.source);
  const sourceFile = path$4.resolve(sourceDir, sourceFileName);
  const extractDir = path$4.resolve(sourceDir, "unpacked");
  const execFile = path$4.resolve(extractDir, mongoFormula.exec);
