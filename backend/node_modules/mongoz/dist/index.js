...
const nodeGracefulShutdown = require('node-graceful-shutdown');
...
const download__default = /*#__PURE__*/_interopDefaultLegacy(download);
const consola__default = /*#__PURE__*/_interopDefaultLegacy(consola);
const decompress__default = /*#__PURE__*/_interopDefaultLegacy(decompress);
...
Object.defineProperty(signals$2,"__esModule",{value:true});signals$2.getSignals=void 0;var _os$1=require("os");
...
Object.defineProperty(main,"__esModule",{value:true});main.signalsByNumber=main.signalsByName=void 0;var _os=require("os");
...
async function startMongo(opts = { args: null }) {
  const platformName = process.platform;
  let platform = mongoFormula.platforms.find((p) => p.name === platformName);
  if (!platform) {
    throw new Error(`Platform '${platformName}' is not available for '${mongoFormula.name}'`);
  }
  const instanceName = "default";
  const appDir = process.env.MONGO_DIR || path__default.resolve(os__default.tmpdir(), mongoFormula.name);
  const dataDir = path__default.resolve(appDir, "data", instanceName, mongoFormula.name);
  const logsDir = path__default.resolve(appDir, "logs", instanceName, mongoFormula.name);
  const logFile = path__default.resolve(logsDir, "logs.txt");
  const sourceDir = path__default.resolve(appDir, "source", mongoFormula.name, mongoFormula.version, platform.name);
  const sourceFileName = `${mongoFormula.name}-${mongoFormula.version}-${platform.name}` + path__default.extname(platform.source);
  const sourceFile = path__default.resolve(sourceDir, sourceFileName);
  const extractDir = path__default.resolve(sourceDir, "unpacked");
  const execFile = path__default.resolve(extractDir, mongoFormula.exec);
  await fsExtra__default.mkdirp(dataDir);
  const spinner = ora__default();
  if (!fsExtra__default.existsSync(extractDir) || !fsExtra__default.existsSync(execFile)) {
    if (!fsExtra__default.existsSync(sourceFile)) {
      const dlMessage = `Downloading ${sourceFileName}`;
      spinner.start(dlMessage + "...");
      const res = download__default(platform.source, sourceDir, { filename: sourceFileName });
    ...
    await decompress__default(sourceFile, extractDir, { strip: 1 });
    spinner.stop();
  }
  await fsExtra__default.mkdirp(logsDir);
  await fsExtra__default.remove(logFile);
  let stdout, stderr;
  const logsFile = await fsExtra__default.open(logFile, "w+");
  stdout = stderr = logsFile;
  consola__default.info(`Writing logs to: ${logFile}`);
  const port = process.env.PORT || mongoFormula.port;
  const execArgs = mongoFormula.execArgs.replace("{port}", port + "").replace("{data}", dataDir).split(" ");
  if (Array.isArray(opts.args)) {
    execArgs.push(...opts.args);
  }
  spinner.info(`Starting ${mongoFormula.name} at port ${port}`);
  const server = execa$1(execFile, execArgs, {
    stdout,
    stderr
  });
  const close = () => Promise.resolve(server.cancel());
  nodeGracefulShutdown.onShutdown(() => close());
  return {
    server,
    close
  };
